...
# def collect_responses(autoencoder: Autoencoder, Z: torch.Tensor, batch_size: int = 128,
#                       device: Optional[str] = None, pbar: Optional[tqdm] = None) -> torch.Tensor:
#     '''
#     Collect responses for a given set of latent samples `Z`.
#     :param autoencoder: used for computing the response (decode + encode)
#     :param Z: latent samples (N, latent_dim)
#     :param batch_size: max batch size
#     :param device: optional device
#     :param pbar: optional progress bar
#     :return: responses (N, latent_dim)
#     '''
#
#     R = []
#
#     if pbar is not None:
#         pbar = pbar(total=len(Z))
#     for z in Z.split(batch_size):
#         if device is not None:
#             z = z.to(device)
#         r = autoencoder.response(z)
...
# def collect_posterior_means(autoencoder: Autoencoder, data: Union[Dataset, torch.Tensor], num_samples: int = 1024, batch_size: int = 128,
#                       device: Optional[str] = None, pbar: Optional[tqdm] = None) -> torch.Tensor:
#     num_samples = min(num_samples, len(data))
#     batches = iter(torch.randperm(len(data)).split(batch_size))
#
#     Zs = []
#     N = 0
#
...
# def collect_reconstructions(autoencoder: Autoencoder, data: Dataset, num_samples: int = 1024, batch_size: int = 128,
#                       device: Optional[str] = None, pbar: Optional[tqdm] = None) -> torch.Tensor:
#     num_samples = min(num_samples, len(data))
#     batches = iter(torch.randperm(len(data)).split(batch_size))
#
...
#         if isinstance(X, tuple):
#             X = X[0]
#         z = autoencoder.reconstruct(X.to(device))
#         if isinstance(z, distrib.Distribution):
#             z = z.mean
#         Zs.append(z.cpu())
#         N += len(batch)
